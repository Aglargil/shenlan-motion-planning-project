...
bool trajGeneration() {
  ...
  /**
   *
   * STEP 2 & 3: Iteratively try path simplification and optimization
   * with decreasing resolution until success or resolution becomes 0
   *
   * **/
  double current_resolution = _path_resolution;
  bool success = false;

  while (current_resolution > 1e-6 &&
         !success) { // 使用一个很小的数而不是0，避免浮点数精度问题
    // 使用当前分辨率简化路径
    auto simplified_path =
        _astar_path_finder->pathSimplify(grid_path, current_resolution);
    MatrixXd path(int(simplified_path.size()), 3);
    ...
    if (trajOptimization(path)) {
      success = true;
      ROS_INFO("Successfully generated trajectory with resolution: %.3f",
               current_resolution);
      break;
    }
    ...
    ROS_WARN("Trajectory generation failed, reducing resolution to: %.3f",
             current_resolution);
  }

  if (!success) {
    ROS_ERROR("Failed to generate trajectory even with minimum resolution");
    return false;
  }
...
bool trajOptimization(Eigen::MatrixXd path) {
  ...
  while (unsafe_segment != -1) {
     ...
     *
     * **/
    repath.conservativeResize(repath.rows() + 1, repath.cols());
    Vector3d start = repath.row(unsafe_segment);
    Vector3d end = repath.row(unsafe_segment + 1);
    if ((start - end).norm() < 0.01) {
      ROS_INFO("start and end are too close, return false");
      return false;
    }
    Vector3d mid = (start + end) / 2;
    for (int i = repath.rows() - 1; i > unsafe_segment + 1; --i) {
      repath.row(i) = repath.row(i - 1);
    }
    repath.row(unsafe_segment + 1) = mid; // 插入中间点
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  double a = _Acc;
  double acc_time = v / a;
  double acc_dist = 0.5 * a * acc_time * acc_time;

  // 使用梯形速度规划计算时间
  auto calc_time = [&](const auto &start, const auto &end) {
    double d = (end - start).norm();
    if (d < 2 * acc_dist) {
      return sqrt(d / a) * 2;
    } else {
      return 2 * acc_time + (d - 2 * acc_dist) / v;
    }
  };

  for (int i = 0; i < Path.rows() - 1; i++) {
    time(i) = calc_time(Path.row(i), Path.row(i + 1));
  }
  return time;
}
